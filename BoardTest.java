...
public class BoardTest {
    ...
    /** A position in which there are 2 regions of black, 2 of white. */
    ...
    public void testNumContig() {
        Board b1 = new Board(BOARD1, BP);
        assertEquals(2, b1.numContig(sq(2,3), _visited, BP));
        assertEquals(0, b1.numContig(sq(2,3), _visited, WP));
        assertEquals(1, b1.numContig(sq(7, 2), _visited, BP));

        Board b2 = new Board(BOARD2, BP);
        assertEquals(9, b2.numContig(sq(2,2), _visited, BP));
        assertEquals(6, b2.numContig(sq(4,2), _visited, WP));
        assertEquals(0, b2.numContig(sq(1, 0), _visited, EMP));

        Board b3 = new Board(BOARD3, BP);
        assertEquals(5, b3.numContig(sq(2,2), _visited, BP));
        assertEquals(11, b3.numContig(sq(1,1), _visited, WP));
    } */

    @Test
    public void testRegionsCount() {
        Board b4 = new Board(BOARD4, BP);
        ArrayList<Integer> testW = new ArrayList<>();
        testW.add(8);
        testW.add(2);
        ArrayList<Integer> testB = new ArrayList<>();
        testB.add(5);
        testB.add(1);

        List<Integer> valueB = b4.getRegionSizes(BP);
        List<Integer> valueW = b4.getRegionSizes(WP);
        ...
        assertEquals("Right region sizes for black?", testB, valueB);
        assertEquals("Right region sizes for black?", testW, valueW);
    }
